'''
Auxiliary functions for PyWake results plotting:
...
def plot_long_WP(data=read_PyWake_out(out_path=OUT_PATH), cst_data=read_CST_out(CST_PATH), flag_compare_cst=True):
...
def plot_long_Z(data=read_PyWake_out(out_path=OUT_PATH), cst_data=read_CST_out(CST_PATH), flag_compare_cst=False, flag_normalize=True):
    # Obtain PyWake variables
    Z=data.get('Z')
    freq=data.get('f')
    ...
    ax=fig.gca()
    ax.plot(freq[ifreq_max]*1e-9, Z[ifreq_max]*norm, marker='o', markersize=4.0, color='blue')
    ax.annotate(str(round(freq[ifreq_max]*1e-9,2))+ ' GHz', xy=(freq[ifreq_max]*1e-9,Z[ifreq_max]*norm), xytext=(-20,5), textcoords='offset points', color='blue') 
...
def plot_trans_WP(data=read_PyWake_out(out_path=OUT_PATH), cst_data=read_CST_out(CST_PATH), flag_compare_cst=True):
    # Obtain PyWake variables
    WPx=data.get('WPx')
    WPy=data.get('WPy')
    s=data.get('s')
...
def plot_trans_Z(data=read_PyWake_out(out_path=OUT_PATH), cst_data=read_CST_out(CST_PATH), flag_compare_cst=True, flag_normalize=True):
    # Obtain PyWake variables
    Zx=data.get('Zx')
    Zy=data.get('Zy')
    freqx=data.get('f')
    ...
    ifreq_x_max=np.argmax(Zx)
    ifreq_y_max=np.argmax(Zy)
    #--- plot Zx⊥(w)
    fig = plt.figure(4, figsize=(6,4), dpi=200, tight_layout=True)
    ax=fig.gca()
    ax.plot(freqx[ifreq_x_max]*1e-9, Zx[ifreq_x_max]*norm_x, marker='o', markersize=4.0, color='green')
    ax.annotate(str(round(freqx[ifreq_x_max]*1e-9,2))+ ' GHz', xy=(freqx[ifreq_x_max]*1e-9,Zx[ifreq_x_max]), xytext=(-10,5), textcoords='offset points', color='grey') 
    ...
    #--- plot Zy⊥(w)
    ax.plot(freqy[ifreq_y_max]*1e-9, Zy[ifreq_y_max]/norm_y, marker='o', markersize=4.0, color='magenta')
    ax.annotate(str(round(freqy[ifreq_y_max]*1e-9,2))+ ' GHz', xy=(freqy[ifreq_y_max]*1e-9,Zy[ifreq_y_max]), xytext=(-10,5), textcoords='offset points', color='grey') 
...
if __name__ == "__main__":
    ...
    out_path=CST_PATH+'runs/cst_quad_dh05'
    plot_PyWake()