...
ixtest=int((xtest-x[0])/dx)+1
iytest=int((ytest-y[0])/dy)+1
print('Fields will be stored in (xtest,ytest): (' + str(round(x[ixtest]*1e3,3)) + ',' + str(round(y[iytest]*1e3,3)) + ') mm')
#-------------#                             
#  time loop  #
#-------------#
...
for n_step in range(tot_nsteps):
    ...
    #store only field around (xtest,ytest)
    Ez_xy=Ez[ixtest-1:ixtest+1,iytest-1:iytest+1,:] #matrix [3,3,nz+1] for each timestep
    ...
    if picmi.warp.me == 0:  #checks if we are in processor 0. Needed for mpi simulations
        ...
        if n_step == 0:
            prefix='0'*5
            hf.create_dataset('Ez_'+prefix+str(n_step), data=Ez_xy)
        else:
...
data = { 'Ez' : Ez_t, #shape=(tot_nsteps,nz)
         'Ex' : Ex_t,
         'Ey' : Ey_t,
         'By' : By_t,
         'Bx' : By_t,
