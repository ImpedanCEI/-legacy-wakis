...
To be launched from the ../WarpX/ folder
...
OUT_FOLDER='out_cub_cav_dipolar/'
...
data = Wsol.read_WarpX_out(out_path=out_path)
...
timestep=np.argmax(charge_dist[len(z)//2, :])           #max at cavity center
lambdas=np.interp(s, z, charge_dist[:,timestep]/q)     #normed charge distribution
