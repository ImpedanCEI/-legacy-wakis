...
def FFT(Xt, dt, fmax=None, r=2.0, flag_zeropadding=True):
    ...
    Xf=2.0*abs(Xfft[mask])/Ns   # Positive FFT, normalized
...
if __name__ == "__main__":
    ...
    ax=fig.gca()
    ax.plot(t, Xt, marker='o', markersize=2.0, color='black', label='Xt1+Xt2+Xt3')
    ax.plot(t, Xt1, marker='o', markersize=2.0, color='blue', label='Xt1')
    ax.plot(t, Xt2, marker='o', markersize=2.0, color='red', label='Xt2')
    ax.plot(t, Xt3, marker='o', markersize=2.0, color='green', label='Xt3')

    ax.grid(True, color='gray', linewidth=0.2)
    ax.legend(loc='best')
    plt.show()
    '''
    Xf, f = FFT(Xt, dt, fmax=0.5, flag_zeropadding=False, r=1.0)
    Xdft, fdft = DFT(Xt, dt, fmax=0.5, Nf=1000)

    # Plot frequency domain
    fig = plt.figure(1, figsize=(6,4), dpi=200, tight_layout=True)
    ax=fig.gca()
    ax.plot(f, Xf, marker='o', markersize=3.0, color='blue', label='FFT')
    ax.plot(fdft, Xdft, marker='o', markersize=3.0, color='red', label='DFT')
    ax.grid(True, color='gray', linewidth=0.2)
    ax.legend(loc='best')
    plt.show()
