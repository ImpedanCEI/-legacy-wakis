...
iz_l1=int((-l1-z_interp[0])/dz_interp)-1
iz_l2=int((l2-z_interp[0])/dz_interp)+1

#--- initialize variables
Ez_dt=np.zeros((nt,nt))  #time derivative of Ez
Ez_dz=np.zeros((nt,nt))  #z spatial derivative of Ez
phi_l1 = np.zeros((int((w_rect*2)/dh + 10 + 1), int((h_rect*2)/dh + 8 + 1), len(s)))  #10 and 8 are the ghost cells added by pyPIC
...
for n in range(len(s)-1):    
    ...
    if t_l1 > 0.0:
        ...
        [Xn,Yn]=np.meshgrid(picFD.yg, picFD.xg)
        smoother=np.exp(-4*np.log(2)*(((Xn-0.0)**2)/w_rect**2+((Yn-0.0)**2)/h_rect**2))    #gaussian smoother in the xy plane
        rho = (Ez_dt[iz_l1,it_l1]/c - Ez_dz[iz_l1, it_l1])*smoother*np.ones_like(picFD.rho)/np.max(smoother)      #this rho is a constant evaluated at z=-l1, t=(s-l1)/c
    ...
    if t_l2 > 0.0:
        ...
        [Xn,Yn]=np.meshgrid(picFD.yg, picFD.xg)
        smoother=np.exp(-4*np.log(2)*(((Xn-0.0)**2)/w_rect**2+((Yn-0.0)**2)/h_rect**2))   #gaussian smoother in the xy plane
        rho = (Ez_dt[iz_l2, it_l2]/c - Ez_dz[iz_l2, it_l2])*smoother*np.ones_like(picFD.rho)/np.max(smoother)         #this rho is a constant size[nx,ny] evaluated at z=l2, t=(s+l2)/c
    ...
    phi_b2=phi_l2[ixtest_phi, iy_b2, n] #phi(x=0, y=b1, s[n])

    Wake_potential[n]=(phi_b1-phi_l1[ixtest_phi, iytest_phi, n])*dt-integral[n]+(phi_l2[ixtest_phi, iytest_phi, n]-phi_b2)*dt
...
ax=fig3.gca()
#ax.plot(s*1.0e3, phi_l1[ixtest_phi, iytest_phi, :]/q, lw=0.5, color='r', label='Phi_l1(s)')
ax.plot(s*1.0e3, (phi_b1-phi_l1[ixtest_phi, iytest_phi, :])/q*dt, lw=1, color='red', label='Phi_b1-Phi_l1(s)')
#ax.plot(s*1.0e3, phi_l2[ixtest_phi, iytest_phi, :]/q, lw=0.5, color='b', label='Phi_l2(s)')
ax.plot(s*1.0e3, (phi_l2[ixtest_phi, iytest_phi, :]-phi_b2)/q*dt, lw=1, color='blue', label='Phi_l2(s)-Phi_b2')
ax.plot(s*1.0e3, ((phi_b1-phi_l1[ixtest_phi, iytest_phi, :])/q+(phi_l2[ixtest_phi, iytest_phi, :]-phi_b2)/q)*dt, lw=1, color='black', label='(Phi_b1-Phi_l1(s))-(Phi_l2(s)-Phi_b2)')
...
ax.plot(Z_freq[0:len(Z)//2], Z[0:len(Z)//2]/norm, lw=1, color='b', label='Z// from numpy FFT')
