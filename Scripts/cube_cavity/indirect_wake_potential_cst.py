...
ns_neg=int(-8.5*sigmaz/c/dt)		#obtains the length of the negative part of s
ns_pos=int(Wake_length/(dt*c))	#obtains the length of the positive part of s
s=np.linspace(-8.5*sigmaz, 0, ns_neg) #sets the values for negative s
...
ax=fig4.gca()
ax.plot(s*1.0e3, Wake_potential*q, lw=1.2, color='orange', label='W_//(s) Warp')
ax.plot(s*1.0e3, integral*q, lw=1.2, color='r', label='W_//(s) integral Warp')
...
ax=fig5.gca()
ax.plot(s*1.0e3, Wake_potential/np.max(Wake_potential), lw=1.2, color='orange', label='W_//(s) Warp')
ax.plot((s-6*sigmaz)*1.0e3, integral/np.max(integral), lw=1.2, color='r', label='W_//(s) integral Warp')
ax.plot(s_cst, Wake_potential_cst/np.max(Wake_potential_cst), lw=1.3, color='black', ls='--', label='W_//(s) CST')
ax.set(title='Longitudinal Wake potential',
        xlabel='s [mm]',
        ylabel='$W_{//}$ [V/pC]',
        )
