...
for n_step in range(tot_nsteps):
    ...
    if !n_insert #a empty vector is considered false. When n_insert receives a value this will be false
        if !rho     #when rho is not empty, store the timestep value
            n_insert=n_step
...
#set up z vector
dz=(zmax-zmin)/nz*np.ones(nz)/unit         #[mm]
z=np.linspace(zmin, zmax, tot_nsteps-t_init[0][0])/unit #[mm]

#set up wakelength and s
t=np.array(t)
z_bunch=(t-t[n_insert])*picmi.constants.c/unit     #position of the source particle q1
WL=50 #wakelength wanted to compute [mm]
n_init=np.array(np.where(z_bunch>WL))   #time to start the wake computation: when z_bunch is longer than the desired wakelength
s=np.linspace(-3*sigmaz, WL, tot_nsteps-t_init[0][0]) #s vector samples the wakelength to be computed [mm]
tau=(z+s)*unit/picmi.constants.c #time of the wake in seconds[s]

#Perform integral
q=1 #charge in eV
wfun=[] #initialize wake vector

for i in range(n_init[0][0], tot_nsteps):  
...
ax2=fig2.gca()
ax2.plot(s, wfun, lw=1.5, color='g')
...
ax3=fig3.gca()
ax3.plot(s, wpot*unit, lw=1.5, color='r', label="analytic")
